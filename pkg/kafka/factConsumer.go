...
func StartFactConsumer(kafkaAddr, topic, dlqtopic, consumerGroup string, cache *cache.RolesCache) {
	ConsumeMessages(kafkaAddr, topic, consumerGroup,
		func(msg Message) error {
			...
			case http.MethodPut:
				err = cache.Update(role, fact.PathRole)
				if err != nil {
					log.Printf("Role %s not changed, err:", fact.PathRole, err)
				}
			case http.MethodDelete:
				err = cache.Delete(fact.PathRole)
				if err != nil {
